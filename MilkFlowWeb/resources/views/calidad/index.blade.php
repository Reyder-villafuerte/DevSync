...
<div class="space-y-6">
    ...
    <x-tabla
        titulo="Historial de evaluaciones Lactoscan"
        descripcion="Densidad, grasa, acidez y agua añadida de cada muestra. Ante una desviación se agenda la visita técnica sola."
        :coleccion="$analyses"
        :columnas="6"
        vacio="No se encontraron análisis que coincidan con los filtros aplicados.">
        ...
        <x-slot:filtros>
            <div class="space-y-3">
                <form method="GET" action="{{ route('calidad.index') }}" class="flex flex-wrap items-center gap-2">
                    <input type="hidden" name="verdict" value="{{ request('verdict') }}">
                    <div class="relative">
                ...
                <div class="flex flex-wrap gap-1.5 items-center">
                    @php
                        $veredictos = [
                            '' => ['Todos', $counts['all']],
                            'conforme' => ['Conformes', $counts['conforme']],
                            'acidez_alta' => ['Acidez alta', $counts['acidez_alta']],
                            'adulterada' => ['Adulterada (agua)', $counts['adulterada']],
                        ];
                        if ($counts['sospechosa'] > 0) {
                            $veredictos['sospechosa'] = ['Sospechosa', $counts['sospechosa']];
                        }
                    @endphp
        ...
        <x-slot:encabezados>
            ...
            <th class="text-left py-3 px-4 font-bold">Parámetros</th>
            <th class="text-left py-3 px-4 font-bold">Veredicto</th>
            <th class="text-left py-3 px-4 font-bold">Cita técnica</th>
        </x-slot:encabezados>

        @foreach($analyses as $ana)
        <tr class="border-b border-slate-50 hover:bg-slate-50/60 transition">
            <td class="py-3 px-4 font-mono text-slate-400">{{ $ana->id }}</td>
            <td class="py-3 px-4 text-slate-500 font-medium">{{ $ana->analysis_date }}</td>
            <td class="py-3 px-4">
                <div class="font-bold text-slate-800">{{ $ana->producer->name }}</div>
                <div class="text-[10px] text-slate-400">DNI: {{ $ana->producer->dni ?: '&mdash;' }}</div>
            </td>
            <td class="py-3 px-4">
                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-600">
                    {{ $ana->producer->zone ? $ana->producer->zone->name : '&mdash;' }}
                </span>
            </td>
            <td class="py-3 px-4 text-[11px] text-slate-600">
                <span class="text-slate-400 font-semibold">G:</span> {{ $ana->fat_percentage ?? '&mdash;' }}% &middot;
...
<div id="modalAnalisis" hidden class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50">
    <div class="bg-white w-full max-w-2xl rounded-3xl p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        <div class="flex items-start justify-between mb-4">
            <div>
                <h3 class="text-base font-bold text-slate-900">Registrar prueba Lactoscan</h3>
                <p class="text-[11px] text-slate-500">Los par&aacute;metros de la muestra; el veredicto y la cita salen solos.</p>
            </div>
            <button type="button" data-cerrar-analisis class="text-slate-400 hover:text-slate-700 text-lg leading-none">&times;</button>
        </div>
...
(function () {
    ...
    document.querySelectorAll('[data-nuevo-analisis]').forEach(function (boton) {
        boton.addEventListener('click', function () { modal.hidden = false; });
    });

    document.querySelectorAll('[data-cerrar-analisis]').forEach(function (boton) {
